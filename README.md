...
- 深度思考和联网搜索：输入框上的两个开关，状态会记住。联网搜索是真的发请求出去查，走 Brave Search，没填 Key 就走免 Key 的那条路。搜不到就说搜不到。
...
- 模型回答：请求发到你自己配的那个接口。
- 联网搜索打开时：请求发到 Brave Search 或 DuckDuckGo。
...
**还没做的功能。** 用 App 控制电脑还没有做。安卓那边的离线音色和操控其他 App，还需要在真机上验证过才算数。
